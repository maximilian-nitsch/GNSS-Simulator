...
def generate_launch_description():
    ...
    config_file_path = os.path.join(
        get_package_share_directory("gnss_simulator_package"),  # noqa
        "config",  # noqa
        "sepentrio_mosaic_h.yaml",  # noqa
    )
    ...
    # Create the node
    gnss_simulator_package_node = Node(
        package="gnss_simulator_package",
        namespace="/auv/gnc/navigation_sensors/gnss",
        executable="gnss_simulator_package_node",
        name="gnss_simulator_package",
        output="screen",
        parameters=[config, {"topic_name": LaunchConfiguration("topic_name")}],
    )

    ld.add_action(gnss_simulator_package_node)
